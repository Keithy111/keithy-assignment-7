...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    # Generate Y using the formula Y = beta0 + beta1 * X + mu + error term
    errors = np.random.normal(0, np.sqrt(sigma2), N)
    Y = beta0 + beta1 * X + mu + errors
    ...
    for _ in range(S):
        ...
        errors_sim = np.random.normal(0, np.sqrt(sigma2), N)
        Y_sim = beta0 + beta1 * X_sim + mu + errors_sim

        # Fit regression model to simulated data
    ...
    plt.figure(figsize=(8, 6))
    plt.hist(slopes, bins=30, alpha=0.7, label='Slopes')
    plt.hist(intercepts, bins=30, alpha=0.7, label='Intercepts')
    plt.title("Histograms of Simulated Slopes and Intercepts")
    plt.xlabel("Value")
    ...
    # Proportion of simulated slopes and intercepts more extreme than the observed
